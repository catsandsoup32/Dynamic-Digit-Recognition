    ...
    def _initialize_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                # Apply He initialization for Leaky ReLU
                init.kaiming_normal_(m.weight, mode='fan_in', nonlinearity='leaky_relu', a=0.01)
...
class CNN_24(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 4, kernel_size=5, stride=1, padding=2, dilation=1) 
        self.conv2 = nn.Conv2d(4, 8, kernel_size=5, stride=1, padding=2, dilation=1) 
    
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2) 

        self.batch_norm1 = nn.BatchNorm2d(num_features=4) # C from (N, C, H, W) filter for more complex feature
        self.batch_norm2 = nn.BatchNorm2d(8)
        self.batch_norm3 = nn.BatchNorm2d(32)

        self.fc1 = nn.Linear(8 * 11 * 11, 128) 
    ...
    def forward(self, x):
        x = self.pool(self.leaky_relu(self.batch_norm1(self.conv1(x)))) # size = (45 + 4 - 5)/1 + 1 = 45 -> 22 after pool
        x = self.pool(self.leaky_relu(self.batch_norm2(self.conv2(x))))          # size = (22 + 4 - 5)/1 + 1 = 22
        # x = self.pool(self.leaky_relu(self.batch_norm3(self.conv3(x)))) # size = (22 + 2 - 3)/1 + 1 = 22 -> 11 after pool
        x = x.view(x.size(0), -1) # flattens the tensor into [batch_size x (128 * 11 * 11)]
        x = self.leaky_relu(self.fc1(x))
        x = self.fc2(x)
...
class CNN_50(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(128 * 11 * 11, 1024) # was at 128 * 11 * 11 
        self.fc2 = nn.Linear(1024, 256)
        self.fc3 = nn.Linear(256, 72) 
